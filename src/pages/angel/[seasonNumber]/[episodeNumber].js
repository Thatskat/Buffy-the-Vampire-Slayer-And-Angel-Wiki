...
const AngelEpisodesPage = ({ episode }) => {
  return <div className="grid episodeInfoPage">
    <Head>
      ...
      <meta name="description" description={`Angel ${episode.seasonNumber} episode ${episode.episodeNumber}`}/>
    </Head>
    <div className='headingSection'>
      <Link href={`/angel/${episode.seasonNumber}`}>Return to all season {episode.seasonNumber} episodes</Link>
...
export const getStaticProps = async (context) => {
  const response = await fetch(
    `http://localhost:3000/api/angel/season/${context.params.seasonNumber.toString()}/`
  );
  const episodes = await response.json();
